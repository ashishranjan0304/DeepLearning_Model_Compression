...
# def prune_multihead_attention_heads(module, heads_to_prune):
#     """Prune specific heads in a MultiheadAttention module."""
#     if isinstance(module, nn.MultiheadAttention):
#         head_dim = module.out_proj.weight.size(1) // module.num_heads
#         mask = torch.ones_like(module.out_proj.weight)

#         for i in range(heads_to_prune):
#             start = i * head_dim
#             end = start + head_dim
#             mask[:, start:end] = 0  # Prune heads

#         prune.custom_from_mask(module.out_proj, name="weight", mask=mask)


# def prune_model_layers(model, prune_amount):
#     """Apply structured pruning to linear and convolutional layers."""
#     for name, module in model.named_modules():
#         if isinstance(module, nn.Linear):
#             prune.ln_structured(module, name="weight", amount=prune_amount, n=2, dim=0)  # Prune structured L2
...
#     """Make pruning permanent by removing hooks."""
#     for name, module in model.named_modules():
#         if hasattr(module, "weight") and prune.is_pruned(module):
#             prune.remove(module, "weight")  # Only remove if pruned
...
def main(args):
    ...
    #     "layer_inter": 1,
    #     "epoch_prune": 1,
    #     "use_cuda": device.type == "cuda",  # Reference device.type instead
    #     "learning_rate": 0.01,
    #     "print_freq": 1,
    # }

    # # Apply additional attributes to args
    # for key, value in additional_args.items():
    #     setattr(args, key, value)  # Add attributes to args
    ...
    # Apply initial pruning to the underlying model
    #prune_amount = 0.1  # Example prune rate

    # Check initial sparsity and parameter count
    # show_sparsity(model_without_ddp)
    # total_params = count_parameters(model_without_ddp)
    # print("Total parameters:", total_params)
    # Prepare Mask object for pruning
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # if (epoch + 1) % 100 == 0:  # Apply pruning every 10th epoch
        #     # Apply iterative pruning on the underlying model
        #     prune_model_layers(model_without_ddp, prune_amount=prune_amount)  # Prune linear/convolutional layers
        #     transformer = model_without_ddp.transformer  # Access transformer from correct reference
        #     for layer in transformer.encoder.layers:
        #         prune_multihead_attention_heads(layer.self_attn, heads_to_prune=2)  # Prune one head
        #     for layer in transformer.decoder.layers:
        #         prune_multihead_attention_heads(layer.self_attn, heads_to_prune=2)  # Prune one head
        #         prune_multihead_attention_heads(layer.multihead_attn, heads_to_prune=2)  # Prune one head

        #     # Calculate and show the final sparsity after training
        #     show_sparsity(model_without_ddp)
        #     # Total number of parameters after pruning
        #     total_params_after = count_parameters(model_without_ddp)
        #     print("Total number of parameters:", total_params_after)
        # Apply FPM pruning every epoch_prune epochs
        # mask.do_mask()  # Apply norm-based pruning
        # mask.do_similar_mask()  # Apply similarity-based pruning
        # model=mask.model
#THIS FORM FPGM
    ...
    # ModelSpeedup(model, torch.rand(10, 1, 28, 28).to(device), mask.mask_index).speedup_model()
    # print(model)
    # Make pruning permanent at the end of training
    #make_pruning_permanent(model_without_ddp)
