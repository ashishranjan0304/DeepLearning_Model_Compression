...
parser.add_argument('--output_dir', default='./logs', type=str, metavar='PATH', help='path to save prune model (default: current directory)')
...
def plot_metrics(metrics, fps, dice_score_end, output_dir, total_time, avg_time_per_image):
    # Convert times to milliseconds
    total_time_ms = total_time * 1000
    avg_time_per_image_ms = avg_time_per_image * 1000

    # Multiply dice score by 100
    dice_score_end = dice_score_end * 100
    ...
    groups = {
        ...
        'Validation Metrics': [
            ('Final Dice Score (%)', dice_score_end)
        ],
        'Evaluation Time (ms)': [
            ('Total Evaluation Time (ms)', total_time_ms),
            ('Time per Image (ms)', avg_time_per_image_ms)
        ]
    }
    ...
    fig, axes = plt.subplots(nrows=3, ncols=2, figsize=(18, 12))  # Adjusted figure size
    fig.suptitle('Model Metrics After Pruning', fontsize=20)

    color_idx = 0

    for ax, (group_name, group_metrics) in zip(axes.flatten()[:5], groups.items()):  # Limiting to 5 plots
...
def evaluate(net, dataloader, device, amp, save_path=None, epoch=0):
    ...
    if hasattr(torch, 'autocast'):
        with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
            for batch_idx, batch in enumerate(tqdm(dataloader, total=num_val_batches, desc='Validation round', unit='batch', leave=False)):
                ...
                mask_true = mask_true.to(device=device, dtype=torch.long)  # Ensure mask_true is LongTensor
                ...
                if model.n_classes == 1:
                    ...
                    total_loss += F.binary_cross_entropy_with_logits(mask_pred, mask_true.float()) + dice_loss(mask_pred, mask_true, multiclass=False)
                else:
                    ...
                    total_loss += F.cross_entropy(mask_pred, mask_true.argmax(dim=1)) + dice_loss(mask_pred, mask_true, multiclass=True)
    ...
    return dice_score / max(num_val_batches, 1), total_loss / max(num_val_batches, 1), total_time, avg_time_per_image
...
def train_model(
    ...
    with tqdm(total=n_train, desc=f'Epoch {epoch}', unit='img') as pbar:
        for batch in train_loader:
            ...
            true_masks = true_masks.to(device=device, dtype=torch.long)  # Ensure true_masks is LongTensor
            ...
            if hasattr(torch, 'autocast'):
                with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
                    ...
                    if model.module.n_classes == 1:  # Access the underlying model
                        ...
                        dice = dice_coeff(F.sigmoid(masks_pred.squeeze(1)), true_masks.float(), reduce_batch_first=False)
                    else:
                        ...
                        dice = multiclass_dice_coeff(F.softmax(masks_pred, dim=1).float(), F.one_hot(true_masks, model.module.n_classes).permute(0, 3, 1, 2).float(), reduce_batch_first=False)
            else:
                masks_pred = model(images)
                if model.module.n_classes == 1:  # Access the underlying model
                    loss = criterion(masks_pred.squeeze(1), true_masks.float())
...
def plot_training_metrics(train_dice_scores, val_dice_scores, train_losses, val_losses, save_path):
    epochs = range(len(train_dice_scores))

    # Move tensors to CPU and convert to numpy if needed
    train_dice_scores = [score.cpu().item() if torch.is_tensor(score) else score for score in train_dice_scores]
    val_dice_scores = [score.cpu().item() if torch.is_tensor(score) else score for score in val_dice_scores]
    ...
    plt.subplot(2, 1, 1)
    plt.plot(epochs, train_dice_scores, label='Training Dice Score')
...
def main():
    ...
    val_acc_1, _, _,_ = evaluate(model, test_loader, device, amp=False)
    ...
    val_acc_2, _, _ ,_= evaluate(model, test_loader, device, amp=False)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_loss, train_dice = train_model(model, train_loader, n_train, optimizer, epoch, log, grad_scaler, device, criterion, dataset)
        train_dice_scores.append(train_dice)
        train_losses.append(train_loss)
        
        prec1, val_loss, _, _ = evaluate(model, test_loader, device, amp=False)
        val_dice_scores.append(prec1.cpu().float())
        ...
        # Add this line to save the plots at the end of each epoch
        plot_training_metrics(train_dice_scores, val_dice_scores, train_losses, val_losses, args.save_path)


    
    _, _, total_eval_time, avg_time_per_image = evaluate(model, test_loader, device, save_path=args.save_path, amp=False)
    ...
    dice_score_end = best_prec1.cpu().float()
    plot_metrics(metrics, fps, dice_score_end, args.save_path, total_eval_time, avg_time_per_image)
